...
impl ConsoleIn {
    ...
    pub fn update_input(&mut self, rl: &mut RaylibHandle) -> bool {
        let inputs_pressed: SmallVec<[_; 8]> = std::iter::from_fn(|| {
            rl.get_char_pressed()
                .map(KeyOrChar::Char)
                .or_else(|| rl.get_key_pressed().map(KeyOrChar::Key))
        })
        .collect();
        ...
        if self.is_focused {
            ...
            if inputs_pressed.is_empty() {
                if let Some((rep_input, ref mut pressed_time)) = self.last_keypress {
                ...
                return inputs_pressed.into_iter().any(|input| {
                    self.last_keypress = Some((input, Instant::now()));
                    self.apply_input(rl, input, is_ctrl_down, is_shift_down, is_alt_down)
                });
            }
        }
