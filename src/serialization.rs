...
pub enum LoadGraphErrorKind {
    ...
    #[error("incompatible version number: {0} (current: {CURRENT_VERSION})")]
    IncompatibleVersion(Version),

    #[error("edge references an unknown vertex")]
    UnknownVertex,
    ...
    DuplicateName { prev: Source<String> },

    #[error("a name (alias or ID) is missing")]
    EmptyName,

    #[error("a name (alias or ID) contains whitespace")]
    InvalidName,

    #[error("position is missing one or more coordinates")]
    IncompletePosition,
    ...
    UsizeToVertexID(#[from] std::num::TryFromIntError),
}
use LoadGraphErrorKind::*;

impl LoadGraphErrorKind {
    pub const fn help(&self) -> &'static str {
        match &self {
            ...
            DuplicateName { .. } => "name is already in use",
...
#[derive(Debug, Error)]
#[error("at line {line}: {kind}\ncode:\n{snippet}", line = src.line + 1, snippet = self.snippet())]
...
impl LoadGraphError {
    ...
    pub fn snippet(&self) -> String {
        let mut snippet = format!(
            "<color = #288cfa>{line:>4} |</color>    <color = #c8c8c8>{src}</color>\
                                  \n     <color = #288cfa>|</color>    {:<space$}{:^<squig$} {help}",
            "",
            "",
            line = self.src.line + 1,
            src = self.src.code,
