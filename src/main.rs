...
fn main() {
    ...
    let font = rl
        .load_font_from_memory(
            &thread,
            ".ttf",
            include_bytes!("resources/ShareTechMono-Regular.ttf"),
            16,
            None,
        )
        .unwrap();
    let mut framebuffer = rl
        ...
        .unwrap();
    let mut shader =
        rl.load_shader_from_memory(&thread, None, Some(include_str!("resources/screen.frag")));
    ...
    'window: while !rl.window_should_close() {
        ...
        if cin.is_focused() {
            ...
            let north = (rl.is_key_down(KeyboardKey::KEY_W) as i8
                - rl.is_key_down(KeyboardKey::KEY_S) as i8) as f32;
            let east = (rl.is_key_down(KeyboardKey::KEY_D) as i8
                - rl.is_key_down(KeyboardKey::KEY_A) as i8) as f32;
            let zoom = (rl.is_key_down(KeyboardKey::KEY_E) as i8
                - rl.is_key_down(KeyboardKey::KEY_Q) as i8) as f32;
            let pitch = (rl.is_key_down(KeyboardKey::KEY_R) as i8
                - rl.is_key_down(KeyboardKey::KEY_F) as i8) as f32;
            let yaw = (rl.is_key_down(KeyboardKey::KEY_X) as i8
                - rl.is_key_down(KeyboardKey::KEY_Z) as i8) as f32;
        ...
        let hovered_vert = data
            ...
            .min_by(|(_, c1), (_, c2)| {
                c1.distance
                    ...
                    .expect("vertices should not have `NaN` distance")
            })
            .map(|(v, _)| v);

        let mouse_snapped = if let &Some(v) = &hovered_vert {
        ...
        if data.is_giving_interactive_targets
            ...
            && let Some(v) = hovered_vert
        {
            if let Some(p) = data.interactive_targets.iter().position(|x| x == &v) {
                console_log!(
                    cout,
                    Info,
                    "removing vertex {} from route",
                    data.graph.vert(v).id
                );
            ...
            rl.draw_texture_mode(&thread, &mut framebuffer, |mut d| {
                ...
                const SCALE_FACTOR: f32 = 1.0 / (10.0 * UPSCALE);
                {
                    ...
                    for (v, vert) in data.graph.verts_iter() {
                        ...
                        let mut color = route
                            .as_ref()
                            .and_then(|route| route.classify(&data.graph, v))
                            .map_or(data.verts_color, |c| match c {
                        ...
                        if data.is_giving_interactive_targets {
                            let is_hovered = get_ray_collision_sphere(
                                d.get_screen_to_world_ray(d.get_mouse_position(), camera),
                                vert.pos,
                                VERTEX_RADIUS,
                            )
                            .hit;
                            ...
                            let is_focused = distance_from_target.dot(distance_from_target)
                        ...
                        {
                            d.draw_capsule(
                                data.graph.vert(p).pos * SCALE_FACTOR,
                                vert.pos * SCALE_FACTOR,
                                1.0 * SCALE_FACTOR,
                                16,
                                0,
                    ...
                    if let Some(route) = &route {
                        for pair in route.result().windows(2) {
                            let [a, b] = pair else {
                                panic!("window(2) should always create 2 elements")
                            };
                            d.draw_capsule(
                                data.graph.vert(*a).pos * SCALE_FACTOR,
                                data.graph.vert(*b).pos * SCALE_FACTOR,
                                2.0 * SCALE_FACTOR,
                                16,
                                0,
                                Color::BLUEVIOLET,
                    ...
                    d.draw_capsule(
                        (data.orbit.target + Vector3::new(-5.0, 0.0, 0.0)) * SCALE_FACTOR,
                        (data.orbit.target + Vector3::new(5.0, 0.0, 0.0)) * SCALE_FACTOR,
                        1.0 * SCALE_FACTOR,
                        16,
                        0,
                    ...
                    d.draw_capsule(
                        (data.orbit.target + Vector3::new(0.0, -5.0, 0.0)) * SCALE_FACTOR,
                        (data.orbit.target + Vector3::new(0.0, 5.0, 0.0)) * SCALE_FACTOR,
                        1.0 * SCALE_FACTOR,
                        16,
                        0,
                ...
                d.draw_rectangle_rec(
                    Rectangle::new(
                        (mouse_snapped.x - 1.5) * UPSCALE,
                        0.0,
                        3.0 * UPSCALE,
                        d.get_screen_height() as f32 * UPSCALE,
                    ),
                ...
                d.draw_rectangle_rec(
                    Rectangle::new(
                        0.0,
                        (mouse_snapped.y - 1.5) * UPSCALE,
                        d.get_screen_width() as f32 * UPSCALE,
                        3.0 * UPSCALE,
                    ),
                    Color::new(255, 255, 255, 32),
                );
                ...
                for (v, vert) in data.graph.verts_iter() {
                    ...
                    d.draw_text_ex(
                        &font,
                        text,
                        (pos - Vector2::new(text_size.x * 0.5, font.baseSize as f32 * 0.5))
                            * UPSCALE,
                        font.baseSize as f32 * UPSCALE,
                        0.0,
                        Color::WHITE,
                    );
                    if let Some(route) = &route
                        && let Some(Visit { distance, parent }) = route.get_visit(v)
                    {
                        ...
                        d.draw_text_ex(
                            &font,
                            &text,
                            (pos + Vector2::new(text_size.x * 0.5 + 3.0, 3.0)) * UPSCALE,
                            font.baseSize as f32 * UPSCALE,
                            0.0,
                            Color::GRAY,
                        );
            ...
            // Console
            {
                if !cin.is_focused() {
                    d.draw_text_ex(
                        ...
                        Vector2::new(
                            SAFE_ZONE,
                            (d.get_screen_height() - font.baseSize * 3) as f32 - SAFE_ZONE,
                        ),
                ...
                d.draw_text_ex(
                    ...
                    Vector2::new(
                        SAFE_ZONE,
                        (d.get_screen_height() - font.baseSize * 2) as f32 - SAFE_ZONE,
                    ),
                    font.baseSize as f32,
                    0.0,
                    Color::GREENYELLOW,
                ...
                d.draw_text_ex(
                    ...
                    Vector2::new(
                        SAFE_ZONE,
                        (d.get_screen_height() - font.baseSize) as f32 - SAFE_ZONE,
                    ),
                    font.baseSize as f32,
                    0.0,
                    Color::GREENYELLOW,
...

                {
                    ...
                    let (selection_range, selection_tail) =
                        (c_in.selection_range(), c_in.selection_tail());

                    if c_out.is_dirty() || c_in.is_dirty() {
                        let (Color { r, g, b, a }, pre) =
                            ConsoleLineCategory::Command.color_prefix();
                        let cmd_string = format!(
                            "<color=rgba({r},{g},{b},{a})>{pre}{}</color>",
                            c_in.current()
                        );

                        let history = c_out
                            .log_history()
                            .flat_map(|msg| msg.lines().enumerate())
                            .chain(
                                data.is_debugging
                                    .then(|| {
                                        c_out.dbg_history().flat_map(|msg| msg.lines().enumerate())
                                    })
                                    .into_iter()
                                    .flatten(),
                            );

                        let mut skip = history.clone().count().saturating_sub(max_history_lines);
                        ...
                        let history = history.map(|(_, s)| s).skip(skip);
...

                    {
                        ...
                        const PREFIX_LEN: usize =
                            ConsoleLineCategory::Command.color_prefix().1.len();
