...
//         /// Display information about commands.
//         help {
//             /// Display general information about all commands.
//             #[print("{msg}")]
//             fn all() -> (msg: String) => {
//                 let cmd_column_width = Cmd::LIST.iter()
...
//                         msg.push(' ');
//                     }
//                     msg.push_str(cmd.help_msg());
//                 }
//                 msg
//             };

//             /// Display more specific information about a specific command.
//             #[print("{msg}")]
//             fn one(command: Cmd) -> (msg: String) => {
//                 let template_column_width = command.usage_list().iter()
//                     .map(|usage| command.input().len() + if !usage.template().is_empty() { usage.template().len() + 1 } else { 0 })
//                     .max()
//                     .unwrap() + 2;

//                 let mut msg = String::with_capacity(2048);
//                 msg.push_str("<color = #48f19d>Usage:</color>");
//                 for usage in command.usage_list() {
//                     msg.push_str("\n    <color = #0096e6>");
...
//         /// Add targets to the current route.
//         sv.route.add {
//             // #[template = ""]
...
//         /// List the results of the current route.
//         sv.route.list {
//             // #[template = ""]
...
//         /// Set the tempo of the route generator.
//         tempo {
...
//         /// Repeat all but the first n arguments.
//         skip {
//             // #[template = "<N> [ANY]..."]
...
//         /// Only repeat the first n arguments.
//         take {
//             // #[template = "<N> [ANY]..."]
...
impl CmdRetDisplay {
    fn display(self, _cout: &mut ConsoleOut, _data: &ProgramData, _args: Vec<String>) {
        todo!()
    }
}
...
impl ParseVert for str {
    ...
    fn parse_vert(&self, graph: &WeightedGraph) -> Result<VertexID, Self::Err> {
        graph.find_vert(self).ok_or_else(|| CmdError::VertexDNE {
            id: self.to_string(),
        })
    }
}
...
pub enum CmdError {
    ...
    #[error("{0}")]
    CheckUsage(clap::Error),

    #[error("vertex \"{id}\" does not exist")]
    VertexDNE { id: String },

    #[error("the name \"{name}\" is already in use by an existing vertex")]
    DuplicateVertexName { name: String },

    #[error("vertices cannot be named \"{name}\". reason: {}", name.reason())]
    BadVertexName { name: ReservedName },
